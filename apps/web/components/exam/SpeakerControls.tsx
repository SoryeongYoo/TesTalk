 ...
 */
export function SpeakerControls({ playCount, isSpeaking, unsupported, onReplay }: SpeakerControlsProps) {
  if (unsupported) {
    return (
  ...
  const canReplay = !isSpeaking && playCount < MAX_PLAYS;
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <div>
        ...
        <p className="mt-1 text-xs font-medium text-slate-400">재생 {playCount} / {MAX_PLAYS}회</p>
      </div>
    </div>
  );
}
